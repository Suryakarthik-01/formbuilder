...
const FormPreviewComponent = ({ form, isPreviewMode = false }) => {
  ...
  const onSubmit = async (data) => {
    if (isPreviewMode) {
      toast.success('This is a preview - form not actually submitted', {
        duration: 2000,
      });
      return;
    }
      ...
      <div key={field.id} className="space-y-2">
        ...
        {field.type === 'text' && (
          <input
            ...
            className={`w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent ${errors[field.id] ? 'border-red-500' : ''}`}
            {...fieldProps}
          />
        )}
        ...
        {field.type === 'textarea' && (
          <textarea
            placeholder={field.placeholder}
            className={`w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent resize-none ${errors[field.id] ? 'border-red-500' : ''}`}
            rows={3}
            {...fieldProps}
          />
        )}
